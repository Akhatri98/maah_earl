...
Conventions (binding for everything Track B emits):
  ...
  * CAPACITY IS INTERNAL IN FORCE, EXTERNAL IN STRESS (R6).  `Capacity` (this
    ...
    (positive). No inertia -> Fd*A with the note "inertia not provided;
    buckling not checked"; exactly one -> Pcr about that axis with a note
    that it is an upper bound (the missing axis could be weaker). Fd is the
    DESIGN stress, `Material.design_stress`: the declared `allowable_stress`
    when the graph carries one, else `yield_strength` (Sprint 4 agreement --
    the 10-bar benchmark checks against a 25 ksi allowable, not the 50 ksi
  ...
  * NUMBERS MUST BE NUMBERS.  A non-finite or non-positive area, yield
    strength, design stress or elastic modulus, a non-finite force, or a
    non-finite safety factor is a ValueError. NaN compares False against
    everything, so "NaN < threshold" would read as PASS -- a NaN must never
    reach a verdict.
...
def member_capacity(graph: DependencyGraph, member_id: str) -> Capacity:
    ...
    PROVIDES (positive). Fd is `Material.design_stress` -- the declared
    allowable when there is one, else yield. Sections with iy = iz = 0 (the
    benchmark's bars) are design-stress-only and say so in the note -- the
    honest, illustrative check plan.md's Scope note allows. A section with
    only one positive inertia (B1) is still checked about that axis: skipping
    the check because the other axis is unknown would hide a real buckling
    failure, so the note says the Pcr is an upper bound instead."""
    ...
    fd = _require_positive_finite(
        material.design_stress, f"material {material.id!r} design stress"
    )
    modulus = _require_positive_finite(
        material.elastic_modulus, f"material {material.id!r} elastic modulus"
    )

    # "yield_force" is the design-stress capacity Fd*A; the name is kept
    # because the governing label the contract sees is "yield" (vs "buckling").
    yield_force = fd * area
