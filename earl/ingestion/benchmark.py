...
# What each variable-table entry actually drives.
#
# DECLARED, not derived -- and it has to be. The truss is generated by a
# custom FeatureScript whose `parameters` list is empty, so the Onshape API
# reports no link between the feature and the variables it reads: the
# dependency lives inside the FeatureScript source. Nothing can be queried to
# recover it, so it is written down here next to the other values CAD does not
# carry, and has to be kept in step with `personal/onshape/truss_10bar.fs`.
#
# A variable edit is the pipeline's primary change signal, and the walker
# seeds from the changed variable, so an entry missing here means a real
# change reaches nothing and the run reports nothing affected.
VARIABLE_DRIVES: dict[str, frozenset[str]] = {
    "barArea": frozenset({"members"}),                 # cross-section of every bar
    "bayWidth": frozenset({"nodes", "members"}),       # node x positions
    "bayHeight": frozenset({"nodes", "members"}),      # node y positions
}


def variable_drives(name: str) -> frozenset[str]:
    """What a variable drives, or an empty set if it is not declared."""
    return VARIABLE_DRIVES.get(name, frozenset())
