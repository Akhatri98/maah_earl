...
def recipient_address(override: str | None = None) -> str:
    load_env()
    return _address(override or os.environ.get("GMAIL_NOTIFY_RECIPIENT", os.environ.get("EARL_NOTIFY_TO", "")),
                    "responsible.engineer@example.com")


def deliver(decision: Decision, run_dir: Path, *, allow_live: bool = False,
            recipient: str | None = None, notice_kind: str | None = None) -> DeliveryReceipt:
    decision.validate()
    load_env()
    ...
    if notice_kind is not None:
        expected = {"ESCALATION": "escalated", "CLEARED": "approved"}
        if notice_kind not in expected or decision.outcome.value != expected[notice_kind]:
            raise ValueError("Agent notice kind does not match the validated Decision")
        subject = f"EARL {notice_kind} | {' '.join(decision.change_description.split())[:120]}"
        notice = ("CLEARED: the current evaluated snapshot passes after a recorded escalation."
                  if notice_kind == "CLEARED" else "AUTONOMOUS ESCALATION: a new structural incident requires review.")
        plain = notice + "\n\n" + plain
        # Fixed code-only notice text; no model controls a delivery classification.
        rendered = rendered.replace("<body>", "<body><p><strong>" + notice + "</strong></p>", 1)
    ...
    attempted, delivered, message_id = False, False, None
    ...
    receipt = DeliveryReceipt(recipient, status, note, subject, str(html_path), str(email_path), str(decision_path),
                              attempted, delivered, message_id)
    try:
        (run_dir / "delivery.json").write_text(json.dumps(receipt.to_dict(), indent=2), encoding="utf-8")
    except OSError:
        if not delivered:
            raise
        LOG.warning("Delivery receipt file unavailable; Gmail acknowledgment retained for the ledger")
    decision.validate()
    return receipt
